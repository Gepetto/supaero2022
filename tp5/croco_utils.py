...
def plotOCSolutions(xs,us,pltAxes=None):
    '''
    Plot xs and us. 
    pltAxes should be a collection of two plt.axes typically produced by a call to plt.subplots.
    By default (none), a new figure is created.

    :param xs: state trajectory.
    :param us: control trajectory.
    :param pltAxes: a collection of 2 plt.axes produced by a call to plt.subplots, None by default.
    '''
    if pltAxes is None:
...
def plotConvergence(log,pltAxes=None):
    ...
    plotConvergenceDetailed(log.costs, log.u_regs, log.x_regs, log.grads,
                            log.stops, log.steps, pltAxes)
    
    
def plotConvergenceDetailed(costs, muLM, muV, gamma, theta, alpha, pltAxes):
    '''
    Plot the cost, regularization <mulM> (in us) and <muV> (in xs), gradient norms <gamma>, 
    stoping criterion <theta>, and line search step <alpha>.
    pltAxes must be a collection of 5 plt.axes typically produced by a call to plt.subplots.
    ...
    :param pltAxes: collection of 5 plt.axes typically produced by a call to plt.subplots.
    '''
