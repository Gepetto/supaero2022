...
class TiagoLoader(object):
    ...
    def __init__(self):
        ...
        if self.srdf_filename:
            ...
            self.q0 = robex.readParamsFromSrdf(
                self.robot.model,
                self.srdf_path,
                self.verbose,
...
def loadTiago(addGazeFrame=False):
    """
    Load a tiago model, without the hand, and with the two following
    modifications wrt example_robot_data.
    - first, the first joint is a planar (x,y,cos,sin) joint, while it is a
    fixed robot in example robot data.
    - second, two visual models of a frame have been added to two new op-frame,
    "tool0" on the robot hand, and "basis0" in front of the basis.
    """
    ...
    eff = np.array([0, 0, 0.08])
    FIDX = robot.model.addFrame(
        pin.Frame("frametool", JIDX, FIDX, pin.SE3(Z, eff), pin.FrameType.OP_FRAME)
    )

    geom.addGeometryObject(
        pin.GeometryObject("axis_x", FIDX, JIDX, cyl, pin.SE3(X, X @ med + eff))
    )
    geom.geometryObjects[-1].meshColor = np.array([1, 0, 0, 1.0])

    geom.addGeometryObject(
        pin.GeometryObject("axis_y", FIDX, JIDX, cyl, pin.SE3(Y, Y @ med + eff))
    )
    geom.geometryObjects[-1].meshColor = np.array([0, 1, 0, 1.0])

    geom.addGeometryObject(
        pin.GeometryObject("axis_z", FIDX, JIDX, cyl, pin.SE3(Z, Z @ med + eff))
    )
    geom.geometryObjects[-1].meshColor = np.array([0, 0, 1, 1.0])
    ...
    eff = np.array([0.3, 0, 0.15])
    FIDX = robot.model.addFrame(
        pin.Frame("framebasis", JIDX, FIDX, pin.SE3(Z, eff), pin.FrameType.OP_FRAME)
    )
    ...
    if addGazeFrame:
        ...
        FIDX = robot.model.addFrame(
            pin.Frame("framegaze", JIDX, FIDX, pin.SE3(Z, eff), pin.FrameType.OP_FRAME)
        )
